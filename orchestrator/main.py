...
async def test_tts_proxy():
    """Proxy to TTS server /test-tts diagnostic endpoint."""
    import httpx
    tts_base = settings.tts_server_url.replace("/synthesize", "")
    async with httpx.AsyncClient(timeout=60.0) as client:
        resp = await client.get(f"{tts_base}/test-tts")
        return JSONResponse(resp.json())


@app.get("/api/test-tts-download/{lang}")
async def test_tts_download(lang: str):
    """Download the test WAV file generated by /api/test-tts."""
    import httpx
    tts_base = settings.tts_server_url.replace("/synthesize", "")
    voices_dir = os.getenv("VOICES_DIR", "/app/voices")
    test_path = f"{voices_dir}/_test_{lang}.wav"

    if not os.path.exists(test_path):
        raise HTTPException(status_code=404, detail=f"Test file not found: {test_path}. Run /api/test-tts first.")

    from starlette.responses import FileResponse
    return FileResponse(test_path, media_type="audio/wav", filename=f"vaani_test_{lang}.wav")
